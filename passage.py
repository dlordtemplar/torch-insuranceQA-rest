...
import whoosh
from flask import (
    Blueprint, request, redirect, url_for, Markup, escape,
    jsonify)
...
def setup_framework():
    ...
    if 'attention_threshold' in request.args:
        attention_threshold = int(float(request.args['attention_threshold'])) / 1000
...
def process_question(received_question, processed_question=None):
    ...
    if 'received_question' in request.args:
        received_question = request.args['received_question']
